...
class UserApi(Resource):  # 声明 User资源
    def get(self):
        # 读取请求参数中的key
    ...
    def post(self):
        ...
        return {"state": 'ok', "msg": '添加{}成功!'.format(name)}

    def delete(self):
        id = request.args.get('id')  # id -> str
        # user = queryById(User, id)  # 是否考虑id 不存在的情况
        # delete(user)
        flag = deleteById(User, id)
        return {'state': 'ok', 'flag': flag, 'msg': '删除{}成功！'.format(id)}
...
class ImageApi(Resource):
    ...
    img_fields = {
        ...
        # 'ima_url': fields.String(attribute='url')
        "url":fields.String,
    ...
    def get(self):
        ...
        if id:
            query(Image).filter(Image.id == id).first()
            # 将对象转成输出的字段格式(json格式)
...
class MusicApi(Resource):
    ...
    parser.add_argument('id', type=int, help='请确定id的参数是否为数值型？')
    ...
    def get(self):
        ...
        # 从 args 中读取name请求参数的值
        name = args.get('name')
        tags = args.get('tag')

        # 从args 中读取session (cookie中的session)
    ...
    class UploadApi(Resource):
        ...
        def post(self):
            ...
            return  {'msg': '上传成功！','path': '/static/upload/{}'.format(newFileName)}

# 将我们的资源添加到api对象中，并声明uri
# -----------------------------------
